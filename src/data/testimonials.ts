...
export const TESTIMONIALS: ReadonlyArray<Testimonial> = [
  {
    id: "renardo-tech.fr",
    quote: {
      fr: "Chez Renardo Tech, on conçoit ddes capteurs d’imagerie de champ électromagnétique pour le diagnostic des réseaux électriques et structures métalliques. Un sujet pointu, qu’il fallait rendre clair et engageant sans en trahir la rigueur, le tout avant nos prochains salons. Studio Tenex a commencé par notre identité, de fond en comble, puis a construit un site qui la porte vraiment : codes visuels taillés pour nous, zéro template recyclé, structure pensée pour notre métier. Ce qui m’a marquée, c’est la réactivité. Site opérationnel en moins de deux semaines. Résultat : on est arrivés aux salons avec un site qui sort du lot. Il nous crédibilise vraiment, rassure nos partenaires, et aujourd’hui on partage notre site avec fierté. Ce n’était pas le cas avant.",
